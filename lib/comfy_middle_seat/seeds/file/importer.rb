...
module ComfyMiddleSeat::Seeds::File
  class Importer < ComfyMiddleSeat::Seeds::Importer
    def initialize(from, to = from)
      super
      self.path = ::File.join(ComfyMiddleSeat.config.seeds_path, from, 'files/')
    end
  ...
  private
    ...
    def save(file, path)
      if file.save
        ComfyMiddleSeat.logger.info("[CMS SEEDS] Imported File \t #{path}")
      else
        ComfyMiddleSeat.logger.warn("[CMS SEEDS] Failed to import File \n#{file.errors.inspect}")
      end
    end
  end
end
