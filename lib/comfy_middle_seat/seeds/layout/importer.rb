...
module ComfyMiddleSeat::Seeds::Layout
  class Importer < ComfyMiddleSeat::Seeds::Importer
    def initialize(from, to = from)
      super
      self.path = ::File.join(ComfyMiddleSeat.config.seeds_path, from, 'layouts/')
    end
    ...
    def import_layout(path, parent)
      ...
      if fresh_seed?(layout, content_path)
        ...
        if layout.save
          ...
          ComfyMiddleSeat.logger.info(message)
        else
          message = "[CMS SEEDS] Failed to import Layout \n#{layout.errors.inspect}"
          ComfyMiddleSeat.logger.warn(message)
        end
      end
