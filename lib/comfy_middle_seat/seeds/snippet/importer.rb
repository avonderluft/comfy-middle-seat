...
module ComfyMiddleSeat::Seeds::Snippet
  class Importer < ComfyMiddleSeat::Seeds::Importer
    def initialize(from, to = from)
      super
      self.path = ::File.join(ComfyMiddleSeat.config.seeds_path, from, 'snippets/')
    end
    ...
    def import!
      Dir.glob("#{path}/*.html").each do |path|
        ...
        if fresh_seed?(snippet, path)
          ...
          if snippet.save
            ComfyMiddleSeat.logger.info("[CMS SEEDS] Imported Snippet \t #{snippet.identifier}")
          else
            ComfyMiddleSeat.logger.warn("[CMS SEEDS] Failed to import Snippet \n#{snippet.errors.inspect}")
          end
        end
